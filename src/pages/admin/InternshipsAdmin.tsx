...
const initialInternship: Omit<Internship, "id" | "created_at"> = {
  company_name: "",
  designation: "",
  total_experience: "",
  from_year: "",
...
const InternshipsAdmin: React.FC = () => {
  const { role } = useAuth();
  const [data, setData] = useState<Internship[]>([]);
  const [form, setForm] = useState(initialInternship);
  ...
  useEffect(() => { fetchData(); }, []);
  const resetForm = () => { setForm(initialInternship); setEditId(null); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editId !== null) {
      const { error } = await supabase.from("internships").update(form).eq("id", editId);
      if (error) setError(error.message); else fetchData();
    } else {
      const { error } = await supabase.from("internships").insert([form]);
      if (error) setError(error.message); else fetchData();
    }
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <h2 className="text-2xl mb-6 font-bold">Internships Admin</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-3 mb-10 bg-gray-100 p-4 rounded-lg">
        ...
        <div className="flex items-center gap-2">
          <button type="submit" className="btn-primary">{editId ? "Update" : "Add"} Internship</button>
